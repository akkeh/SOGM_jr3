...
ODF::ODF(unsigned long N, unsigned long M, unsigned long bins, unsigned long H, float tth, float tinhibTh, unsigned long tinhibRel) {
...
float* derv(float* x, unsigned long N) {
    float* y = new float[N];
    y[0] = 0;
    for(unsigned long n=1; n<N; ++n)
        y[n] = x[n] - x[n - 1];
...
float** nd_derv(float** x, unsigned long M, unsigned long N) {
    float** y = new float*[M];
    for(unsigned long m=0; m<M; ++m)
        y[m] = derv(x[m], N);
    return y;
...
float* ODF::phaseFlux(float* x, unsigned long N, float th, float inhibTh, unsigned long inhibRel) {
    ...
    float** derv = nd_derv(pX_unwrap, FFTSIZE, frames);
